...
public class DuplicateNumbers {
    ...
    public static void main(String[] args) {
        int nums[]={12,7,3,3,1,2,12,12,1,1,1,1,1,1,1,1,1};
        //i will sort this array 1,1,1,3,3,7,12,12,12
        Arrays.sort(nums);
        //Creating List of Integer
        List<Integer> uniqueInteger=new ArrayList<>();
        
        for(int i=0;i<nums.length-1;i++) {//nums.lenght-1 =9.maximum i=8
//          System.out.print(nums[i] + " ");
            if(nums[i]==nums[i+1]&&!uniqueInteger.contains(nums[i])) {//i+1=9
